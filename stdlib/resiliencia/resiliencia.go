...
func init() {
	ptst.RegistraModuloImpl(&ptst.ModuloImpl{
		...
		Metodos: []*ptst.Metodo{
			ptst.NewMetodoOuPanic("novo_disjuntor", met_novo_disjuntor, "Cria disjuntor(limite, timeout_segundos)"),
			ptst.NewMetodoOuPanic("novo_limite_de_taxa", met_novo_limite, "Cria limite de taxa(max_tokens, tokens_por_segundo)"),
			ptst.NewMetodoOuPanic("nova_retentativa", met_nova_retentativa, "Cria retentativa(tentativas, base_ms, fator)"),
		},
	})
}

var ModuloResiliencia = &ptst.ModuloImpl{
	Info: ptst.ModuloInfo{
		Nome: "resiliencia",
...
func met_novo_disjuntor(_ ptst.Objeto, args ptst.Tupla) (ptst.Objeto, error) {
	if err := ptst.VerificaNumeroArgumentos("novo_disjuntor", false, args, 2, 2); err != nil {
		return nil, err
	}
	limite, err := ptst.NewInteiro(args[0])
	if err != nil {
		return nil, err
	}
	timeout, err := ptst.NewDecimal(args[1])
	if err != nil {
		return nil, err
	}
	d := NovoDisjuntor(int(limite.(ptst.Inteiro)), float64(timeout.(ptst.Decimal)))
	return ptst.Texto(d.estado), nil
}

func met_novo_limite(_ ptst.Objeto, args ptst.Tupla) (ptst.Objeto, error) {
	if err := ptst.VerificaNumeroArgumentos("novo_limite_de_taxa", false, args, 2, 2); err != nil {
		return nil, err
	}
	max, err := ptst.NewDecimal(args[0])
	if err != nil {
		return nil, err
	}
	porSeg, err := ptst.NewDecimal(args[1])
	if err != nil {
		return nil, err
	}
	_ = NovoLimiteDeTaxa(float64(max.(ptst.Decimal)), float64(porSeg.(ptst.Decimal)))
	return ptst.Verdadeiro, nil
}

func met_nova_retentativa(_ ptst.Objeto, args ptst.Tupla) (ptst.Objeto, error) {
	if err := ptst.VerificaNumeroArgumentos("nova_retentativa", false, args, 0, 3); err != nil {
		return nil, err
	}
	r := NovaRetentativa()
	if len(args) >= 1 {
		t, err := ptst.NewInteiro(args[0])
		if err == nil {
			r.Tentativas = int(t.(ptst.Inteiro))
		}
	}
	if len(args) >= 2 {
		b, err := ptst.NewDecimal(args[1])
		if err == nil {
			r.BaseEsperaMs = float64(b.(ptst.Decimal))
		}
	}
	if len(args) >= 3 {
		f, err := ptst.NewDecimal(args[2])
		if err == nil {
			r.FatorBackoff = float64(f.(ptst.Decimal))
		}
	}
	return ptst.Verdadeiro, nil
}
