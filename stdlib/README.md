...
A **Biblioteca Padrão** (do inglês, _Standard Library_ ou simplesmente `stdlib`) do **Harpia** é um conjunto de módulos utilitários robustos implementados diretamente em Go. Eles são expostos nativamente para os scripts Harpia de forma embutida ou via importação explícita.
...
- **Tabela de Módulos Globais**: Dentro do `init()`, cada módulo se autopopula com suas funções nativas e constantes, e se registra na máquina virtual chamando a função centralizada `hrp.RegistraModuloImpl()`.
