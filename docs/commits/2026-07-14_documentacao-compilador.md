...
> Nesta sessão, realizamos o maior esforço de documentação técnica do Harpia, adicionando comentários inline ricos no padrão GoDoc em português brasileiro para absolutamente todos os arquivos-fonte lógicos do repositório (pacotes `cmd`, `compartilhado`, `playground`, `stdlib`, `lexer`, `parser` e `ptst`). 
> 
