...
// ResolveArquivohrp localiza e calcula o caminho absoluto de arquivos físicos de scripts ou módulos no disco rígido.
//
...
//   - Se a pasta mapeada contiver um arquivo de entrada mestre 'inicio.hrp' dentro dela, resolve importando-o;
//   - Se o caminho referenciado omitir a extensão física, tenta anexar e buscar arquivos compilados '.so'
//     e, em caso de insucesso, busca arquivos com a extensão '.hrp' (extensão antiga) ou '.pt' de forma transparente.
func ResolveArquivohrp(caminhoArqOuMod string, bases []string, curDir string) (string, error) {
