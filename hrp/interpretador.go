...
// visiteChamadaFuncao avalia todos os argumentos passados na chamada e invoca o chamável via hrp.Chamar.
...
func (i *Interpretador) visiteTenteCapture(node *parser.TenteCaptureFinalmente) (resultado Objeto, errFinal error) {
	...
	hrpErr, ok := errFinal.(*Erro)
	if !ok {
		hrpErr = NewErro(RuntimeErro, Texto(errFinal.Error()))
	}
	...
	escopoCapture.DefinirSimbolo(NewVarSimbolo(node.NomeErro, hrpErr))
